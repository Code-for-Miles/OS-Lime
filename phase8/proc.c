...
void ChildCode() {
  ...
  Write(device, "\n\r");
  Sleep(pid);
  cons_printf("In ChildCode, about to exit");
  Exit(pid*5);
  cons_printf("In childCode, done exiting!");
}

void ChldHandler() {
  int pid, cpid, ec, device;
  char str[3];
  cons_printf("In ChldHandler, about to wait!");
  cpid =  Wait(&ec);
  cons_printf("In ChldHandler, done waiting!");
  pid = GetPid();
