...
void SignalISR() {
  if(pcb[cur_pid].TF_p->ebx == SIGINT) // sig_num is in ebx 
     pcb[cur_pid].siginit_handler_p = (func_p_t)pcb[cur_pid].TF_p->ecx;
  else if(pcb[cur_pid].TF_p->ebx == SIGCHLD)
    pcb[cur_pid].sigchld_handler_p = (func_p_t)pcb[cur_pid].TF_p->ecx;
}
...
void WrapperISR(int pid, func_p_t handler_p) {
  ...
  p1 = &pcb[pid].TF_p->efl;      //reference to first 4bytes
  p2 = &pcb[pid].TF_p->cs;      //reference to nxt 4bytes
...
void ForkISR() {
  ...
  MemCpy(stack[cpid], stack[cur_pid], STACK_SIZE);
  distance = (int)(stack[cpid] - stack[cur_pid]);
  (int)pcb[cpid].TF_p += distance;
  pcb[cpid].TF_p->ebx = 0;
  pcb[cpid].TF_p->esp += distance;
  pcb[cpid].TF_p->ebp += distance;
  pcb[cpid].TF_p->esi += distance;
  pcb[cpid].TF_p->edi += distance;
...
void WaitISR() {
  int i,cpid, *ec_p, found;
  found = 0;
  for(i=0; i<PROC_MAX; i++) {
    if((pcb[i].state == ZOMBIE) && (pcb[i].ppid == cur_pid)) {
      found =1;
      cpid = i;
      break;  
    }
...
void ExecISR(void) {
  ...
  //buildng stack page.
  Bzero((char *)pages[page_index[1]].addr, PAGE_SIZE);
  p = (int *)(pages[page_index[1]].addr + PAGE_SIZE); //outside of page
  p--; //4kb - 1bye = 4092.
  ...
  *p = 0;
  ...
  pcb[cur_pid].TF_p--; // reduce p by a whole TF_t
  
  pcb[cur_pid].TF_p->efl = EF_DEFAULT_VALUE | EF_INTR; // enables intrrupts
  pcb[cur_pid].TF_p->cs = get_cs();                    // duplicate from CPU
  pcb[cur_pid].TF_p->eip = pages[page_index[0]].addr; // set to code page
}
