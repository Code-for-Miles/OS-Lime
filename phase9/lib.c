...
void ReclaimPages(int pid) {
  ...
  for(i = 0; i<PAGE_MAX; i++) {
    pages[i].owner = pages[i].owner == pid?-1:pages[i].owner;
  }
}
...
int Alloc(int pid, int how_many, int page_index[]) {
  ...
  for(i=0; i<PAGE_MAX;i++) {
    if(pages[i].owner == -1) {
       ...
       page_index[count] = i;
       count++;
       if(count == how_many) return 0; // successfully allocated. 
    }
  }
  if(count < how_many) { //not enough unowned pages.
    for(i=0;i<PAGE_MAX; i++) {
      if(pages[i].owner == pid) pages[i].owner = -1; //reclaim.
    }
  }
  return -1;
}
